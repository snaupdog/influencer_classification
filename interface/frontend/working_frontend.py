...
DEFAULT_CSV_PATH = "top100_with_sentiment.csv"
    ...
    <script>
        ...
        document.getElementById('upload-form').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const response = await fetch("{{ url_for('forward_to_laptop_b') }}", {
                    method: 'POST',
                    body: formData
                });
...
# --- ROUTE TO FORWARD FILE TO LAPTOP B ---

LAPTOP_B_URL = "http://10.1.7.87:5001/receive"  # TODO: replace with actual Laptop B IP


@app.route("/forward-to-laptop-b", methods=["POST"])
def forward_to_laptop_b():
    if "file" not in request.files:
        return jsonify({"success": False, "message": "No file uploaded"}), 400
    file = request.files["file"]
    try:
        response = requests.post(
            LAPTOP_B_URL, files={"file": (file.filename, file.stream, "text/plain")}
        )
        return (
            jsonify({"success": True, "message": response.text}),
